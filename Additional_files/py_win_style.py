...
Author: Akash Bora
Version: 1.8
Fixed: Unicode filename support in drag-and-drop
"""
...
try:
    ...
    from ctypes import (POINTER, Structure, byref, c_int, pointer, sizeof,
                        windll, c_buffer, WINFUNCTYPE, c_uint64, create_unicode_buffer)
...
class apply_dnd():
    ...
    def __init__(self, widget: int, func: Callable, char_limit: int=260) -> None:
        ...
        WM_DROP_FILES = 0x233
        GWL_WND_PROC = -4
        # Use DragQueryFileW for Unicode support
        ...
        def py_drop_func(hwnd, msg, wp, lp):
            ...
            if msg == WM_DROP_FILES:
                ...
                # Use create_unicode_buffer for Unicode filenames
                file_buffer = create_unicode_buffer(char_limit)
                files = []
                for i in range(count):
                    func_DragQueryFile(typ(wp), i, file_buffer, sizeof(file_buffer))
                    # No need to decode - Unicode buffer returns string directly
