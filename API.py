...
class KompasAPI:
    def __init__(self, kompas_document=None, file_address=None):
...
class CreateDxf:
    def __init__(self, file_address=None):
        ...
        if file_address is None:
            ...
            if splitext(file_address)[1] != ".m3d": pass
            api = KompasAPI(file_address=file_address)
        ...
        sleep(1)    # надо для того, что бы камера успела навестись в положение "Нормально к..",
                    # иначе вид получается промежуточный
